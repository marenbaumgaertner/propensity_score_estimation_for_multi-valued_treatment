...
## Calibration plots
```{r}
make_calibtation_plot(results$adaboost$predictions, as_tibble(W_test), "adaboost")
...
make_calibtation_plot(results$forest_grf$predictions, as_tibble(W_test), "Random Forest")
...
for (classifier in classifiers) {
  ...
  predictions = predictions %>% as_tibble()
  if (colnames(predictions)[1] == "V1"){
    colnames(predictions) <- c("1", "2", "3", "4", "5")
  }
  
   # Reorder the columns in the dataframe
  predictions <- predictions[, c("1", "2", "3", "4", "5"), drop = FALSE]
