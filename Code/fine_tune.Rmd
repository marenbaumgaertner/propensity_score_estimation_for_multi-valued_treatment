...
random_sample_indices <- sample(nrow(xgboost_method$grid), 1000, replace = FALSE)
...
```{r, message=FALSE}
best_logit <- finetuning_soft_classifier(x=X, y=W , method=multinomial_logit) #parses params

best_logit_nnet <- finetuning_soft_classifier(x=X , y=W , method=multinomial_logit_nnet) #parses params

best_knn <- finetuning_soft_classifier(x=X, y=W , method=knn) #parses params

#best_knn_radius <- finetuning_soft_classifier(x=X, y=W , method=knn_radius)

best_nb_bernulli <- finetuning_soft_classifier(x=X, y=W , method=nb_bernulli) #parses params

#best_nb_gaussian <- finetuning_soft_classifier(x=X, y=W , method=nb_gaussian)

best_probability_forest_grf <- finetuning_soft_classifier(x=X, y=W, method=probability_forest_grf) #parses params

best_probability_forest_ranger <- finetuning_soft_classifier(x=X, y=W, method=probability_forest_ranger) #parses params

best_mlpc <- finetuning_soft_classifier(x=X, y=W, method=mlpc) #parses params
...
```{r}
tuning_results <- list(best_knn, best_logit, best_logit_nnet, best_nb_bernulli, best_probability_forest_grf, best_probability_forest_ranger, best_mlpc, best_xgboost)
names(tuning_results)[1:8] <- c("knn", "logit", "logit_nnet", "nb_bernulli", "probability_forest", "ranger", "mlpc", "xgboost")
...
tuning_results['xgboost'] <- list(best_xgboost)
```
...
t <- length(unique(W))
t <- 50
random_guess <- ((1-(1/t))^2 + (t-1)*((1/t)^2) )/t

tuning_results$logit$best_params
tuning_results$logit_nnet$best_params 
tuning_results$knn$best_params
tuning_results$nb_bernulli$best_params
tuning_results$probability_forest$best_params
tuning_results$ranger$best_params
tuning_results$mlpc$best_params
tuning_results$xgboost$best_params


m <- xgboost(data = X[fold != f,], label = W[fold != f]-1, nrounds = 10, max_depth=10)
m_docall <- do.call(xgboost)
m <- xgboost_fit(x= X[fold != f,], y = W[fold != f], args=list(nrounds=10))
```
